            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-10 mt-10">
                {
                    events.map(event => <Link key={event.id} to={`/${event.id}`}>
                        <div className="flex flex-col rounded-xl justify-center   border-2 border-solid rounded-t-xl">
                            <figure>
                                <img src={event.background_image} alt={event.title} className="rounded-xl h-60" />
                            </figure>
                            <div>
                            <div className="items-center text-left">
                                <h2 className="text-2xl text-black font-bold p-4 mb-5">{event.title}</h2>
                                <div className="flex justify-between px-3 text  items-center ">
